...
def run_chat_sequence(messages, functions):
    if "live_chat_history" not in st.session_state:
        st.session_state["live_chat_history"] = [{"role": "assistant", "content": "Hello! I'm Sheldon Cooper, how can I assist you?"}]
    ...
    if "Glad to help" in assistant_message['content']:
    ...
    if assistant_message.get("function_call"):
        ...
        internal_chat_history.append({"role": "user", "content": "You are a data analyst - provide personalized/customized explanations on what the results provided means and link them to the the context of the user query using clear, concise words in a user-friendly way. Or answer the question provided by the user in a helpful manner - either way, make sure your responses are human-like and relate to the initial user input. If the user says 'thanks', then reply 'Glad to help, May the force be with YOU!' If the user says 'thank you', then reply 'Glad to help, May the force be with YOU!'. Your answers must not exceed 600 characters"})
        ...
        print(results)
        # content = tt.to_string(results, style=tt.styles.ascii_thin_double)
        # content = None
        ...
        # print(content)
        import pandas as pd
        df = pd.DataFrame(eval(results))
        print(df)

        print(type(results))
        print(st.session_state["live_chat_history"][-1])
        print(type(st.session_state["live_chat_history"][-1]['content']))

        response = {"content": st.session_state["live_chat_history"][-1]['content']}
        return response
