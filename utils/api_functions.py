...
def send_api_request_to_openai_api(messages, functions=None, function_call=None, model=AI_MODEL, openai_api_key=OPENAI_API_KEY):
    ...
    try:
        ...
        messages[0]["type"] = "object"
        json_data = {"model": "gpt-4o-mini", "store": True, "messages": messages}
        # json_data = {"model": "gpt-4o-mini", "messages": messages}

        if functions: 
            json_data.update({"functions": functions})
        if function_call: 
            json_data.update({"function_call": function_call})
        print("send_api_request_to_openai_api triggered")
